...
def get_optimizer(minet, args):
    ...
    logger.info(f' lr warmup epochs: {args.lr_warmup_epochs}')
...
def train(model, miset, **kwargs):
    ...
    if len(model.train_loss):
        train_loss_per_recap = pd.concat(model.train_loss, axis=0, ignore_index=True)
        if 'recap' in train_loss_per_recap:
            train_loss_per_recap = train_loss_per_recap.groupby('recap', as_index=False).mean()
        else:
            train_loss_per_recap['recap'] = 0
        train_loss_per_epoch = train_loss_per_recap.groupby('epoch', as_index=False).mean()
    else:
    ...
    if len(model.valid_trail.eval_loss):
        valid_loss_per_recap = pd.concat(model.valid_trail.eval_loss, axis=0, ignore_index=True)
        if 'batch' in valid_loss_per_recap:
            valid_loss_per_recap = valid_loss_per_recap.groupby('batch', as_index=False).mean()
        else:
            valid_loss_per_recap['batch'] = 0
        valid_loss_per_epoch = valid_loss_per_recap.groupby('epoch', as_index=False).mean()
    else:
    ...
    if len(model.test_trail.eval_loss):
        test_loss_per_recap = pd.concat(model.test_trail.eval_loss, axis=0, ignore_index=True)
        if 'batch' in test_loss_per_recap:
            test_loss_per_recap = test_loss_per_recap.groupby('batch', as_index=False).mean()
        else:
            test_loss_per_recap['batch'] = 0
        test_loss_per_epoch = test_loss_per_recap.groupby('epoch', as_index=False).mean()
    else:
    ...
    for iepoch in range(args.num_epochs):
        ...
        # Both could apply if args.num_epochs < lr_warmup_epochs + lr_cooldn_epochs!!!
        new_lr = new_lr if iepoch >= args.lr_warmup_epochs else \
                args.learning_rate * (iepoch + 1) / args.lr_warmup_epochs

        new_lr = new_lr if (iepoch + args.lr_cooldn_epochs) < args.num_epochs else \
        ...
        if new_lr is None:
            if iepoch > args.lr_warmup_epochs and not isinstance(model.lr_scheduler, float):
        ...
        ####### Tweak Loss Function #######
        # apply loss cooldown epochs
        if args.loss_cooldn_epochs > 0 and iepoch > args.lr_warmup_epochs:

            _cooldn_factor = max([0, args.loss_cooldn_epochs + args.lr_warmup_epochs - iepoch]) / \
                            args.loss_cooldn_epochs
        ...
        _train_loss_mean = loss_this_epoch.groupby('recap', as_index=False).mean()
        ...
        if model.valid_trail.epoch == model.epoch:
            valid_loss_per_recap = pd.concat([valid_loss_per_recap, model.valid_trail.eval_loss[-1].groupby('batch', as_index=False).mean()], axis=0, ignore_index=True)
        ...
        if model.test_trail.epoch == model.epoch:
            test_loss_per_recap = pd.concat([test_loss_per_recap, model.test_trail.eval_loss[-1].groupby('batch', as_index=False).mean()], axis=0, ignore_index=True)
...
def save_model_optim(model, save_dir=Path.cwd()):
    ...
    try:
        ...
        logger.error(f'Error in copying net_src_file and args: {err}')
...
def load_state_dict(model, load_dir=Path.cwd(), load_log=True):
    ...
    if load_log and train_loss_file.exists():
        ...
        if 'recap' in model.train_loss:
            model.train_loss = model.train_loss.astype({'recap':np.int32}, errors='ignore')
        else:
            model.train_loss['recap'] = model.train_loss.index
    ...
    if load_log and eval_loss_file.exists():
        ...
        if 'recap' in eval_loss:
            eval_loss = eval_loss.astype({'recap':np.int32}, errors='ignore')
        else:
            eval_loss['recap'] = eval_loss.index
        eval_loss = eval_loss.astype({'idx':np.int32, 'recap': np.int32,
            'len':np.int32, 'loss':np.float32, 'loss_std': np.float32}, errors='ignore')
...
def evaluate_in_train(model=None, miset=None, trail=misc.Struct(), **kwargs):
    ...
    if save_chkpt and eval_trail.save_dir and eval_trail.times_called > 1:
        ...
        # save the last model in the parent directory
        save_state_dict(model, save_dir=eval_trail.save_dir)
...
def load_train_valid_test_dataset(args, return_midat=False):
    ...
    if valid_data is not None and len(valid_data) > 0:
        ...
        valid_set = get_dataset(valid_data, args, jit_upsample=None, jit_downsample=None)
    ...
    if test_data is not None and len(test_data) > 0:
        ...
        # jit_loader = args.jit_loader # do this if you want to preload all evaluation dataset
        test_set = get_dataset(test_data, args, jit_upsample=None, jit_downsample=None)
        # args.jit_loader = jit_loader
        # valid_loader = get_dataloader(valid_set, shuffle=False,
        #             batch_size=min([args.batch_size, len(valid_set)]),
        #             drop_last=False)
...
def launch_evaluate(args):
    ...
    for idat, midat in enumerate(midats):
        ...
        meta_cols2loss = [_col for _col in ['idx', 'moltype', 'file', 'db', 'id', 'cluster', 'hasNaN', 'isaOutlier'] if _col in midat]
...
def get_args(argv=None):
    ...
    if args.save_dir in [None, '', False]:
        ...
        if args.save_dir_prefix:
            args.save_dir = f'{args.save_dir_prefix}' + str(args.save_dir)
        if args.save_dir_suffix:
            args.save_dir = str(args.save_dir) + f'{args.save_dir_suffix}'
