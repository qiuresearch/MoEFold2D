...
def dihedral_btwn_quads(xyz_quads, return_radian=False, epsilon=1e-11):
    """ 
    calculate the dihedral/torsion angle by four points about the 2-3 axis 
    specifically, the angle between the 1->2 vector and the 3->4 vector
    or between the 4->3 vector and the 2->1 vector
    
    the xyz must be a np.ndarray, while quad_xyzs can be nested with list/tuples
    for example, [[xyz1], [xyz2, xyz3, xyz4]] is supported

    return angles in degree by default
    """
    xyz_quads = misc.unpack_list_tuple(*xyz_quads)
    assert len(xyz_quads) == 4, f"len(quad_xyzs) must be 4, not {len(xyz_quads)}!!!"

    b1 = xyz_quads[1] - xyz_quads[0]
    b2 = xyz_quads[2] - xyz_quads[1]
    b3 = xyz_quads[3] - xyz_quads[2]

    # enforce unit vectors:
    b1 = b1 / (np.linalg.norm(b1) + epsilon) # np.sqrt(b1.dot(b1))
    b2 = b2 / (np.linalg.norm(b2) + epsilon) # np.sqrt(b2.dot(b2))
    b3 = b3 / (np.linalg.norm(b3) + epsilon) # np.sqrt(b3.dot(b3))
...
def show_ncp_geometry(all_ncp_plot_vars):
    ...
    ncp_colors = [gwplot.qual_color(0), gwplot.qual_color(6),
                  gwplot.qual_color(8), gwplot.qual_color(2)]
    axes_colors = [gwplot.qual_color(9), gwplot.qual_color(1)]
    limits = []
            ...
            if i == 0:
                ...
                if j == 1:
                    ...
                    ax.plot(stack_vec[:, 0], stack_vec[:, 1], stack_vec[:, 2],
                            c=gwplot.qual_color(4), linewidth=3, label='Stack Axes')
                    ax.plot(all_origins[:, 0], all_origins[:, 1],
                            all_origins[:, 2], linewidth=2, c=gwplot.qual_color(3),
    ...
    ax.plot([center[0]], [center[1]], [center[2]], marker='*',
            c=gwplot.qual_color(7), ms=10, label='Array Center', linestyle='None')
    ...
    limits.append(stack_vec)
    ax.plot(stack_vec[:, 0], stack_vec[:, 1], stack_vec[:, 2],
            c=gwplot.qual_color(4), linewidth=3)
