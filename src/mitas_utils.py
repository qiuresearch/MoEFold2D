...
def parse_args(*argv):
    ...
    paparser.add_argument(*flex_var('-argv'), metavar='', type=str, default='-h', help='string of argv from command line (default: auto-assigned)')
    paparser.add_argument(*flex_var('-kwargs'), metavar='', type=dict, default=dict(), help='dict of argv from command line (default: auto-assigned)')
    paparser.add_argument(*flex_var('-verbose'), metavar='', choices=[0,1,2], default=1, type=int, help="('_') (default: 1)")
    paparser.add_argument(*flex_var('-job_genre'), metavar='', type=str, default=None, help='only used for auto-generating directory names (default: None)')
    paparser.add_argument(*flex_var('-objective'), metavar='', type=str, default='loss', help='metric name for saving checkpoints (default: loss)')
    ...
    paparser.add_argument(*flex_var('-profiler'), metavar='', type=misc.str2bool, nargs='?', const=True, default=False, help='turn on profiler')
    ...
    paparser.add_argument(*flex_var('-sys'), metavar='', type=str,  default=f'{os.uname().sysname}', help='auto generated worker info')
    paparser.add_argument(*flex_var('-host'), metavar='', type=str,  default=f'{os.uname().nodename}', help='auto generated worker info')
    paparser.add_argument(*flex_var('-time'), metavar='', type=str,  default=f'{datetime.now().strftime("%Y%m%d-%H%M%S")}', help='auto generated worker info')
    paparser.add_argument(*flex_var('-home_dir'), metavar='', type=str, default=os.getcwd(), help='auto-generated current directory')
    paparser.add_argument(*flex_var('-run_src_file'), metavar='', type=str, default=None, help="the main run code (auto-assigned)")
    paparser.add_argument(*flex_var('-net_src_file'), type=str, default=None, metavar='', help="python code for net definitions (auto-assigned)")
    paparser.add_argument(*flex_var('-params'), type=dict, default=None, metavar='', help="the number of parameters (auto-generated)")
    paparser.add_argument(*flex_var('-email'), metavar='', type=str, default=None, help='the email address to be notified after done')
    paparser.add_argument(*flex_var('-log'), type=str, default='fly_paddle.log', metavar='log_file', help="the log file")

    # two ways to load args: 1) configuration file via -config, 2) args.json in load_dir
    paparser.add_argument(*flex_var('-config'), metavar='json_file', type=str, default=None, help='args config file to load (default: None)')
    ...
    paparser.add_argument(*flex_var('-load_dir'), metavar='', type=str, default=None,  help="for loading model args, src file, and states (default: None)")
    ...
    paparser.add_argument(*flex_var('-train_data'), type=str, default=None, metavar='', help="train data file name (split into train/eval/test according to split_valid and split_test)")
    paparser.add_argument(*flex_var('-valid_data'), type=str, default=None, metavar='', help="valid data file name")
    paparser.add_argument(*flex_var('-test_data'), type=str, default=None, metavar='', help="test data file name")
    ...
    paparser.add_argument(*flex_var('-data_len'), type=int, nargs='+', default=[0], metavar='', help="[[min, max], pad_flag(0: nothing; -1: max_len; >1: the max)]")
    paparser.add_argument(*flex_var('-data_include'), type=str, nargs='+', default=None, metavar='', help="include certain col values only (e.g., col_name col_values...)")
    paparser.add_argument(*flex_var('-data_exclude'), type=str, nargs='+', default=None, metavar='', help="exclude certain col values (e.g., col_name col_values...)")
    paparser.add_argument(*flex_var('-data_range'), type=str, nargs='+', default=None, metavar='', help="select by col value range (e.g., lenCT 4 1000)")
    paparser.add_argument(*flex_var('-data_ratio'), type=str, nargs='+', default=None, metavar='', help="select by col/col valule ratio (e.g., lenCT len 0.04 1)")
    paparser.add_argument(*flex_var('-data_size'), type=int, default=None, metavar='', help="the number of data to use if > 0")
    paparser.add_argument(*flex_var('-data_sampling'), type=str, default='stratify', metavar='', help="how to sample data_size (stratify/random)")

    paparser.add_argument(*flex_var('-split_valid'), type=float, default=0.0, metavar='', help="fraction of validation data used by train_test_split()")
    paparser.add_argument(*flex_var('-split_test'), type=float, default=0.0, metavar='', help="fraction of test data used by train_test_split()")
    ...
    paparser.add_argument(*flex_var('-split_bucket_num'), type=int, default=11, metavar='', help='number of buckets for stratified split')

    # for input data
    paparser.add_argument(*flex_var('-input_genre'), type=str, nargs='+', default=['seq2onehot'], metavar='', help="seq2onehot+bpmat+quant+...")
    paparser.add_argument(*flex_var('-feats_nn'), type=int, default=0, metavar='', help="add nearest neighbor features to input (for seq2onehot only)")
    paparser.add_argument(*flex_var('-feats_dbn'), type=misc.str2bool, metavar='', nargs='?', const=True, default=False, help="add dbn features to input (for seq2onehot only)")
    paparser.add_argument(*flex_var('-feats_attr'), type=misc.str2bool, metavar='', nargs='?', const=True, default=False, help="add attribute features to input (for seq2onehot only)")
    paparser.add_argument(*flex_var('-feats_extra'), type=misc.str2bool, metavar='', nargs='?', const=True, default=False, help="add extra features to input")
    paparser.add_argument(*flex_var('-input_fmt'), type=str, nargs='+', default=None, metavar='', help="the matrix fmt of input: NL/NLL/... (not used yet)")
    paparser.add_argument(*flex_var('-input_dim'), type=int, default=None, metavar='', help="the channel dimensions of inputs (only for the first for now)")
    ...
    paparser.add_argument(*flex_var('-label_fmt'), metavar='', type=str, nargs='+', default=['NL'], help="label fmt: NL/NLL/... (not used yet)")
    paparser.add_argument(*flex_var('-label_dim'), metavar='', type=int, nargs='+', default=None, help="the model output dimensions of the labels")
    # additional labels -- None at this point

    # for output used by evaluate and predict (work in progress)
    ...
    paparser.add_argument(*flex_var('-embed_act_fn'), type=str, default=None, metavar='', help="only when embed_fn != embed")
    paparser.add_argument(*flex_var('-embed_norm_fn'), type=str, default=None, metavar='', help="only when embed_fn != embed")
    ...
    paparser.add_argument(*flex_var('-seq2mat_args'), type=str, default='------- seq2mat_castxd args', metavar='', help="------- set2mat_castxd args")    
    paparser.add_argument(*flex_var('-seq2mat_method'), metavar='', type=str, default='concat', help='how to convert sequence to matrix')
    paparser.add_argument(*flex_var('-seq2mat_in_linear'), type=misc.str2bool, metavar='', nargs='?', const=True, default=False, help='apply linear to input before seq2mat')
    paparser.add_argument(*flex_var('-seq2mat_out_dim'), type=int, default=None, metavar='', help="the output dim after seq2mat (default: None)")
    paparser.add_argument(*flex_var('-seq2mat_act_fn'), type=str, default=None, metavar='', help="activation function for the matrix (default: None)")
    paparser.add_argument(*flex_var('-seq2mat_norm_fn'), type=str, default=None, metavar='', help="normalization function for the matrix (default: None)")
    ...
    paparser.add_argument(*flex_var('-loss_recap_interval'), type=int, default=None, metavar='', help="the recap interval for loss functions")
    ...
    paparser.add_argument(*flex_var('-loss_with_padding'), type=misc.str2bool, metavar='', nargs='?', const=False, default=False, help="whether to include padding in loss")
    ...
    paparser.add_argument(*flex_var('-loss_cooldn_epochs'), metavar='', type=int, default=0, help="cool down loss function continuousely (function specific...)")
    ...
    paparser.add_argument(*flex_var('-lr_warmup_epochs'), type=int, default=7, metavar='', help='# of lr warmup epochs at the beginning (from args.learning_rate/epochs)')
    paparser.add_argument(*flex_var('-lr_cooldn_epochs'), type=int, default=7, metavar='', help='# of lr cooldn epochs at the end (to args.learning_rate/epochs)')
    ...
    paparser.add_argument(*flex_var('-jit_upsample'), type=str, default=None, metavar='', help="the category col to upsample minority classes")
    paparser.add_argument(*flex_var('-jit_downsample'), type=str, default=None, metavar='', help="the category col to downsample majority classes")
    paparser.add_argument(*flex_var('-train_size'), type=int, default=None, metavar='', help="used in train() for training data")
    paparser.add_argument(*flex_var('-valid_size'), type=int, default=None, metavar='', help="used in evaluate() for evaluation data")
    paparser.add_argument(*flex_var('-test_size'), type=int, default=None, metavar='', help="used in evaluate() for test data")
    paparser.add_argument(*flex_var('-predict_size'),  type=int, default=None, metavar='', help="used in predict() for prediction data")
    ...
    paparser.add_argument(*flex_var('-num_valids_per_epoch'), type=int, default=1, metavar='', help="# of valid checkpoints per epoch")
    paparser.add_argument(*flex_var('-num_tests_per_epoch'), type=int, default=1, metavar='', help="# of test checkpoints per epoch")
    ...
    paparser.add_argument(*flex_var('-valid_callback'), type=str, default=None, metavar='', help="auto-assigned")
    paparser.add_argument(*flex_var('-test_callback'), type=str, default=None, metavar='', help="auto-assigned")
    paparser.add_argument(*flex_var('-chkpt_save_limit'), type=int, default=7, metavar='', help='maximum of saved checkpoint states')

    paparser.add_argument(*flex_var('-earlystop_rubric'), type=str, nargs='+', default=None, metavar='', 
        choices=['trainloss', 'validloss', 'lossgap', 'lossratio', 'trainobj', 'validobj', 'objgap', 'objratio', 'pgscore'], help="earlystop methods")
    paparser.add_argument(*flex_var('-earlystop_delay'), type=int, default=22, metavar='', help="the number of epochs after which for earlystop to kick in")
    paparser.add_argument(*flex_var('-earlystop_patience'), type=int, default=7, metavar='', help="the number of most recent epochs to check for earlystop condition")
    paparser.add_argument(*flex_var('-earlystop_minval'), type=float, default=None, metavar='', help="the minimum value for the rubric")
    paparser.add_argument(*flex_var('-earlystop_maxval'), type=float, default=None, metavar='', help="the maximum value for the rubric")
    paparser.add_argument(*flex_var('-earlystop_mindif'), type=float, default=None, metavar='', help="minimum absolute difference")
    paparser.add_argument(*flex_var('-earlystop_maxdif'), type=float, default=None, metavar='', help="maximum absolute difference")
    paparser.add_argument(*flex_var('-earlystop_minpct'), type=float, default=None, metavar='', help="minimum percentage change")
    paparser.add_argument(*flex_var('-earlystop_maxpct'), type=float, default=None, metavar='', help="maximum percentage change")    

    # paparser.add_argument(*flex_var('-trainobj_rdiff'), type=float, default=7e-3, metavar='', help="relative difference")
    # paparser.add_argument(*flex_var('-trainobj_patience'), type=int, default=7, metavar='', help="relative difference patience")
    # paparser.add_argument(*flex_var('-validobj_rdiff'), type=float, default=7e-3, metavar='', help="relative difference")
    # paparser.add_argument(*flex_var('-validobj_patience'), type=int, default=7, metavar='', help="relative difference patience")        

    # paparser.add_argument(*flex_var('-trainloss_rdiff'), type=float, default=7e-3, metavar='', help="relative difference")
    # paparser.add_argument(*flex_var('-trainloss_patience'), type=int, default=7, metavar='', help="relative difference patience")
    # paparser.add_argument(*flex_var('-validloss_rdiff'), type=float, default=7e-3, metavar='', help="relative difference")
    # paparser.add_argument(*flex_var('-validloss_patience'), type=int, default=7, metavar='', help="relative difference patience")
    ...
    paparser.add_argument(*flex_var('-scheduler'), type=str, nargs='?', const=None, default=None, metavar='', help='a json file containing a list of dicts')

    # pre-defined settings
    # paparser.add_argument(type=str, default='======= moody args =======', metavar='', help="======= moody args =======")
    ...
    subparser = subparsers.add_parser('cross_validate', parents=[paparser], description='', help='cross-validate the model', formatter_class=formatter)
    subparser.set_defaults()
    ...
    subparser.add_argument("eval_files", type=str, nargs='+', metavar='', help="input fasta files for validation")
    subparser.add_argument(*flex_var("-split_data"), type=misc.str2bool, metavar='', nargs='?', const=True, default=False, help="split data into train/valid/test datasets")
    ...
    values - v1,v2,..vn (n>=2) for serial search (default) or grid_search
             ...
             min,max,[step] for bayes_search (step is only used for int and logint)
    ''')
...
def autoconfig_args(args):
    ...
    for i, label_genre in enumerate(args.label_genre):
        ...
        elif args.label_genre[i] in ['idist']:
            if args.label_fmt is None: args.label_fmt = 'NLL1'

        elif args.label_genre[i] in ['fdist']:
            if args.label_fmt is None: args.label_fmt = 'NLL'
    ...
    if args.width > 0:
        ...
        if args.return_dim is None:
            args.return_dim = [args.width] * 3
...
def idx2onehot(idx, ntype=None, padding_idx=None, dtype=np.int32):
    """ convert idx to one-hot """
    if ntype is None:
        ntype = idx.max() + 1

    identity_mat = np.identity(ntype, dtype=dtype)

    if padding_idx is None:
        onehot = identity_mat[idx]
    else:
        onehot = np.zeros(list(idx.shape) + [ntype], dtype=dtype)
        onehot[idx != padding_idx] = identity_mat[idx[idx != padding_idx]]
...
def calc_conv_padding(kernel_size, stride=1, dilation=1):
    """ the goal is to keep num_channels constant """
    return ((dilation * (kernel_size - 1) + 1) - stride) / 2

calc_padding = calc_conv_padding
...
def load_trim_midat(args=misc.Struct(), **kwargs):
    r"""Load midata (default a dict or dataframe) from args.data_name (default 'train')
    ...
    # ct data sometimes have wrong 'len' in the header (usually incomplete sequences)
    ...
    #    1) data_len as [min, max, pad_flag]
    #    2) data_len as [pad_flag] (all sequences used)
    ...
    if args.data_include is not None and len(args.data_include) > 1:
        if args.data_include[1].lower() in ['none', 'null', 'na', 'all']:
            logger.info(f'Skipping data_include: {args.data_include}...')
        else:
            logger.info(f'Applying data_include: {args.data_include}...')
            if df[args.data_include[0]].dtype == np.int64:
                df = df[df[args.data_include[0]].isin([int(_) for _ in args.data_include[1:]])]
            elif df[args.data_include[0]].dtype == np.float64:
                df = df[df[args.data_include[0]].isin([float(_) for _ in args.data_include[1:]])]
            else:
    ...
    if args.data_exclude is not None and len(args.data_exclude) > 1:
        if args.data_exclude[1].lower() in ['none', 'null', 'na', 'all']:
            logger.info(f'Skipping data_exclude: {args.data_exclude}...')
        else:
            logger.info(f'Applying data_exclude: {args.data_exclude}...')
            if df[args.data_exclude[0]].dtype == np.int64:
                df = df[~ df[args.data_exclude[0]].isin([int(_) for _ in args.data_exclude[1:]])]
            elif df[args.data_exclude[0]].dtype == np.float64:
                df = df[~ df[args.data_exclude[0]].isin([float(_) for _ in args.data_exclude[1:]])]
            else:
    ...
    if args.data_size is not None and args.data_size > 0:
        if df.shape[0] >= args.data_size:
            ...
            if args.data_sampling in ['random']:
                df = df.sample(args.data_size)
            elif args.data_sampling in ['stratify']:
                df, _ = brew_midat.split_midat(df, fraction=float(args.data_size)/len(df), stratify=args.split_stratify,
                    bucket_key=args.split_bucket_key, bucket_num=args.split_bucket_num, shuffle=False,
                    random_state=args.split_seed, save_pkl=False, save_json=False, save_prefixes=False, quiet=args.verbose)
            else:
                logger.error(f'Unsupported data sampling method: {args.data_sampling}, skipping!!!')
        else:
...
def bake_midat_ab2021(df, args=misc.Struct(), **kwargs):
    ...
    if args.batch_size > 1:
        ...
        input_data = [misc.fix_length2d(_v, [args.max_len, _ncols]) for _v in input_data]
    ...
    for input_genre in args.input_genre:
        if input_genre.startswith(('seq2onehot', 'onehot', 'seq2quant', 'quant')):
            ...
            if args.feats_extra:
                ...
                input_feats = [np.concatenate((_v, misc.fix_length(df.iloc[_i]['extra'], len(_v), skip_dims=1)),
            ...
            if input_fix2len > 0:
                pad_func = functools.partial(misc.fix_length, length=input_fix2len, skip_dims=[1])
            ...
            if input_fix2len > 0:
                ...
                pad_func = functools.partial(misc.fix_length, length=input_fix2len, skip_dims=[0])
                input_feats = map_fn(pad_func, input_feats, desc='Applying fix_length')
            ...
            if input_fix2len > 0:
                pad_func = functools.partial(misc.fix_length2d, mode='constant', constant_values=((0, 0), (0, 0)))
                bpmat = starmap_fn(
                    pad_func,
    ...
    for label_genre in args.label_genre:
        if label_genre in ['upp']:
            ...
            if input_fix2len > 0:
                ...
                pad_func = functools.partial(misc.fix_length1d, mode='constant', constant_values=(0.0,0.0))
                label_y = starmap_fn(pad_func, zip(label_y, [input_fix2len] * num_seqs),
                                        desc='Fixing 1D length')

            midat.append(label_y)

        elif label_genre in ['dbn']: # each seq gives Lx3 matrix
            logger.error(f'label_genre: {label_genre} not yet implemented!')
            ...
            masks = [(_angle != 1000.0).astype(_angle.dtype) for _angle in angles] # 1 for valid angles
            ...
            if input_fix2len > 0:
                ...
                pad_func = functools.partial(misc.fix_length, length=input_fix2len, skip_dims=[1,2], constant_values=0.0)
                label_y = map_fn(pad_func, label_y, desc='Applying fix_length to tangle')

            midat.append(label_y)

        elif label_genre in ['idist']:
            if 'dist' not in df:
                logger.warning(f'Cannot find dist in data! (ignore if predicting)')
                continue

            label_y = map_fn(functools.partial(molstru.mat2quant, start=2, end=34, width=1,
                underflow=True, overflow=True, onehot=False, keepdim=True), df['dist'])

            midat.append(label_y)

        elif label_genre in ['dist']:
            if 'dist' not in df:
                logger.warning(f'Cannot find dist in data! (ignore if predicting)')
                continue

            label_y = df['dist'].to_list()
...
def pfarm_metric(guess, label, mask=None, keep_batchdim=True, beta=1.0, threshold=None,
        epsilon=1e-9, overflow=0.5, return_dict=False):
    ...
    if keep_batchdim:
        ...
        data_size = guess[0].size if mask is None else mask[0].sum()
    else:
        sum_axis = None # tuple(range(0, guess.ndim))
        data_size = guess.size if mask is None else mask.sum()
    ...
    ##### the rationale is that the values are usually scalars or
    ##### arrays of batch_size after the sum_axis operation

    # deal with paddle.Tensor and torch.tensor
    if type(p) not in (np.ndarray, np.float64, np.float32, np.int64, float, int):
        if hasattr(data_size, 'cpu'):
            data_size = data_size.cpu()
        if hasattr(p, 'cpu'): 
            p = p.cpu()
            pp = pp.cpu()
            tp = tp.cpu()

        if hasattr(p, 'float'): # to deal with torch.Tensor
         # to deal with torch.Tensor
            data_size = np.array(data_size.float())
            p = np.array(p.float())
            pp = np.array(pp.float())
            tp = np.array(tp.float())
        elif hasattr(p, 'numpy'): # to deal with paddle.Tensor
            if hasattr(data_size, 'numpy'):
                data_size = data_size.numpy()
            p = p.numpy()
            pp = pp.numpy()
            tp = tp.numpy()
        else:
            logger.warning(f'Cannot convert {p} to np.ndarray, use original type {type(p)}')
        # data_size = np.array(data_size.astype('float32'))
        # p = np.array(p.astype('float32'))
        # pp = np.array(pp.astype('float32'))
        # tp = np.array(tp.astype('float32'))
    ...
    precision = np.nan_to_num(precision, copy=False, nan=0.0, posinf=0.0, neginf=0.0)
    fscore = np.nan_to_num(fscore, copy=False, nan=0.0, posinf=0.0, neginf=0.0)
    recall = np.nan_to_num(recall, copy=False, nan=0.0, posinf=0.0, neginf=0.0)
    mcc = np.nan_to_num(mcc, copy=False, nan=0.0, posinf=0.0, neginf=0.0)
    acc = np.nan_to_num(acc, copy=False, nan=0.0, posinf=0.0, neginf=0.0)
...
def pfarm_variance(guess, label=None, keep_batchdim=True, beta=1.0, threshold=0.5, epsilon=1e-11):
...
def save_all_results(midata, save_prefix, save_dir=None, 
                    csv_exclude=None, # use default if None
                    pkl_exclude=None, # use default if None
    ...
    if csv_exclude is None:
        csv_exclude = ['seq', 'ct', 'ct_guess', 'ct_label',
                   'single', 'pair', 'msa',
                   'ctmat', 'ctmat_guess', 'ctmat_label',
                   'bp', 'bpmat', 'bpmat_guess', 'bpmat_label',
                   'ppmat', 'ppmat_logits', 'ppmat_guess', 'ppmat_label',
                   'dbn', 'dbn_logits', 'topology', 'topology_logits',
                   'upp', 'upp_guess', 'upp_label',
                   'tangle', 'tangle_guess', 'tangle_label',
                   'dist', 'idist', 'idist_guess', 'idist_label',
                   'dihedral', 'dihedral_guess', 'dihedral_label',
                   ]
    if pkl_exclude is None:
        pkl_exclude = ['bpmat', 'bpmat_guess', 'bpmat_label',
                   'ctmat', 'ctmat_guess', 'ctmat_label',
                   ]
