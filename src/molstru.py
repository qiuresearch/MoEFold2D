...
res2idx_dict = dict() # defaultdict(lambda: '-')
res2idx_dict.update({
    '-': 0,
    'A': 1,
    'U': 2,
...
idx2res_dict = dict() # defaultdict(lambda: 0)
idx2res_dict.update(dict([(_v, _k) for _k, _v in idx2res_dict.items()]))
...
def seq2idx(seq, dbn=None, use_nn=0, use_dbn=False, length=None):
    ...
    for i in range(use_nn, min([length, seq_len]) + use_nn):
        ...
        res_emb = [res2idx_dict.get(seq[i + _i], 5) for _i in nn_indices]
...
def seq2idx_old(seq, dbn=None, use_nn=0, use_dbn=False, length=None):
    """ use a single number for each seq/dbn """
    if not length:
        length = len(seq)
...
def seq2RNA(
        ...
        upper=True,
        vocab_set=set('AUCG'),
        trans_fn=molstru_config.get_res2RNA_trans_dict(return_fn=True),
        pseudo_random=False,
        trans_dict=str.maketrans(molstru_config.get_res2RNA_trans_dict()), # only used when pseudo_random=True
        verify=False):
    ...
    if pseudo_random:
        ...
        seq_out = ''.join([trans_fn[_s]() for _s in seq_in])
...
def DEBUG_encoding():
    ...
    print(seq2idx(seq_debug))
    print('\nQuant/scalar encoding fixed length=23')
    print(seq2idx(seq_debug, length=23))
    print('\nQuant/scalar encoding with dbn')
    print(seq2idx(seq_debug, dbn_debug, use_dbn=True, length=23))
...
moltype_mapping = misc.DefaultDict(lambda moltype: moltype)
moltype_mapping.update({
    ...
    "Group I Introns": 'gpI-intron', # bpRNA
    'grp2': 'gpII-intron',  # archiveII
    "group_II_intron": 'gpII-intron', # Stralign
    "Group II Intro or Group II Introns": 'gpII-intron', # bpRNA
...
def parse_refseq_id(refseq_id, fmt='refseq_rna'):
    """ parse of the id field of rna sequences and areturn a dict """
    # one example:
    # NM_000056.5 Homo sapiens keto acid dehydrogenase(BCKDHB), transcript variant 2, mRNA; nuclear gene for mitochondrial product 
    tokens = refseq_id.split('; ')
    subtokens = tokens[0].split(', ')
    return {
        'id': subtokens[0].split()[0], # NM_000056.5
        'moltype': subtokens[-1], # mRNA
        'molfunc': tokens[-1] if len(tokens) > 1 else '', # nuclear gene for mitochondrial product 
        'title': ', '.join(subtokens[:-1]), # NM_000056.5 Homo sapiens keto acid dehydrogenase(BCKDHB), transcript variant 2
    }
    
    
def parse_json_lines(json_info, fmt='rnacentral', return_list=True):
    """ 
    Parse RNAcentral json files
    """
    if (isinstance(json_info, str) and os.path.exists(json_info)) or isinstance(json_info, Path):
        fname = str(json_info)
        with open(fname, 'r', encoding='utf-8') as iofile:
...
def compose_ct2bpseq_lines(ct, seq, id=None, header=False, return_list=True):
    ...
    if header and isinstance(id, str):
...
def compose_stockholm_lines(seq, id='noname', wussn=None):
    """ wussn: Washington University Secondary Structure Notation """
    id = id.replace(" ", "_")
    sto_lines = ['# STOCKHOLM 1.0']

    for i in range(0, len(seq), 50):
        sto_lines.append('')
        sto_lines.append(f'{id}         {seq[i:i+50]}')
        if wussn:
            sto_lines.append(f'#=GR {id} SS {wussn[i:i+50]}')
...
def compose_ctmat2bpseq_lines(ctmat, seq, id=None, header=False, return_list=True):
    """ bpseq fmt: resnum resname resnum_bp (0 if unpaired)
    """
...
def parse_seq_lines(seq_info, fmt='fasta', has_id=True, has_seq=True, has_dbn=None, has_upp=None,
    ...
    """ 
    Designed to parse all sequences in ONE file only

    Supported fmts: fasta, dbn, seqres, seq (ArchiveII dataset)
    
    Returns a list of dict by default
        return_namedtuple=True returns a list of namedtuple
    ...
    if fmt == 'seqres':
        seq_lines = [_s for _s in seq_lines if _s != '--']

    num_lines = len(seq_lines)
    if not mpi.current_process().daemon:
        logger.info(f'Total number of lines: {num_lines} in file: {fname}')
    
    fmt = fmt.lower()
    if fmt in ['fasta', 'dbn', 'seqres', 'refseq']:
        id_tag = '>'
    elif fmt == 'seq':
        id_tag = ';'
    else:
    ...
    seq_starts = np.array([_i for _i in range(num_lines) if seq_lines[_i][0] == id_tag] \
                    + [num_lines], dtype=int)
    num_seqs = len(seq_starts) - 1

    # use multiprocessing if too many sequences
    if num_seqs > 3000 and not mpi.current_process().daemon:
        return misc.mpi_map(functools.partial(parse_seq_lines, fmt=fmt, has_id=has_id, has_dbn=has_dbn,
            has_upp=has_upp, return_namedtuple=return_namedtuple, return_list=return_list),
            [seq_lines[seq_starts[i]:seq_starts[min([i+3000, num_seqs])]] for i in range(0, num_seqs+1, 3000)],
            quiet=False, desc=f'Parsing {num_lines} seq lines')
    ...
    for i, iline in enumerate(seq_starts[:-1]):
        ...
        if has_dbn:
            ...
            assert len(seq_dict['dbn']) == len(seq_dict['seq']), \
                f'Length of dbn: {len(seq_dict["dbn"])} != length of seq: {len(seq_dict["seq"])} for file: {fname}'
    ...
    if return_namedtuple:
        SeqTuple = namedtuple('SeqTuple', ['fname', 'id', 'seq', 'dbn', 'upp'], defaults=[None]*5)
        return [SeqTuple(**_d) for _d in seqs_dict]
    elif return_list:
...
def parse_stockholm_lines(seq_info, fmt='sto', return_dict=True, return_namedtuple=False, return_list=False):
    """ 
    Designed to parse all sequences in ONE file only

    Supported fmts: Stockholm 1.0
    ...
    if isinstance(seq_info, Path) or isinstance(seq_info, str):
        fname = str(seq_info)
        seq_lines = get_file_lines(fname, keep_empty=False, strip=True, comment=None, keep_comment=True)
    else:
        fname = ''
        seq_lines = seq_info
    ...
    # use multiprocessing if too many lines
    if num_lines > 6000000 and not mpi.current_process().daemon:
        return misc.mpi_map(functools.partial(
            parse_stockholm_lines, fmt=fmt, return_namedtuple=return_namedtuple, return_list=return_list),
            [seq_lines[i:i+6000] for i in range(0, num_lines, 6000)],
            quiet=False, desc=f'Parsing {num_lines} seq lines')
    
    fmt = fmt.lower()
    if fmt in ['sto', 'stk', 'stockholm']:
        id_tag = '# STOCKHOLM 1.0'
    else:
        id_tag = None
        logger.critical(f'Unknow sequence file format: {fmt}')

    # find all lines with id tags ------ [num_lines] is added!!!
    seq_starts = np.array([_i for _i, _linestr in enumerate(seq_lines) if _linestr.startswith(id_tag)] \
                    + [num_lines], dtype=int)
    ...
    if len(seq_starts) == 1:
        ...
        # remove consecutive lines with the id tag (only applicable for SEQ format)
        nlines_per_seq = seq_starts[1:] - seq_starts[:-1]
        idx_invalid_seq = np.where(nlines_per_seq < 2)[0]

    seqs_dict = []

    # a slow way...
    for iseq, istart in enumerate(seq_starts[:-1]):
        if iseq in idx_invalid_seq: continue

        iend = seq_starts[iseq + 1]

        seq_dict = dict(fname=fname, id='', seq='', dbn='', ct=np.empty((0,2), dtype=np.int32))

        for linestr in seq_lines[istart+1:iend-1]: # -1 to skip the //
            if not linestr.startswith('#'): # sequence lines
                tokens = linestr.split()
                if len(tokens) != 2:
                    logger.error(f'Incorrect number of tokens: {tokens}')
                    continue
                if seq_dict['id'] and seq_dict['id'] != tokens[0]:
                   logger.error(f'Only one seq name is supported but more are given: {seq_dict["id"]} vs {tokens[0]}') 
                else:
                    seq_dict['id'] = tokens[0]
                    seq_dict['seq'] += tokens[1]
    
            elif linestr.startswith('#=GR'): # Stockholm mark-up lines
                tokens = linestr.split()
                if len(tokens) != 4:
                    logger.error(f'Incorrect number of tokens: {tokens}')
                    continue                
                if seq_dict['id'] and seq_dict['id'] != tokens[1]:
                   logger.error(f'Only one seq name is supported but more are given: {seq_dict["id"]} vs {tokens[1]}') 
                else:
                    seq_dict['id'] = tokens[1]
                    if tokens[2] == 'SS': # _: hairpin loops; -: 
                        seq_dict['dbn'] += tokens[3]                
                    else:
                        logger.warning(f'Unsupported feature type: {tokens[2]}')

            elif linestr.startswith('#'): # comment
                pass
            else: # may check the existence of //
                pass
    ...
    if return_dict:
        return seqs_dict
    elif return_namedtuple:
        SeqTuple = namedtuple('SeqTuple', ['fname', 'id', 'seq', 'dbn'], defaults=[None]*4)
        return [SeqTuple(**_d) for _d in seqs_dict]
    elif return_list:
        return [list(_d.values()) for _d in seqs_dict]
    else:
...
def parse_st_lines(seq_info, fmt='sta', seq=True, dbn=True, ssn=True, nkn=True,
    ...
    for iseq, iline in enumerate(seq_starts[:-1]):
        ...
        if fmt == 'sta': # sta format doesnot have the list of structure motifs at the end
            if iline != iend:
                logger.warning(f'Next line: {iline} should be equal to iend: {iend} for fmt: {fmt}!')
...
def parse_ct_lines(ct_info, return_list=False):
    """ parse ct file lines for ONE molecule into a list or a dict """
    ...
    for i, s in enumerate(dbn):
        ...
        if s in '._-,:~':
            ...
            if len(shut_stack[s]):
                ...
                logger.debug(f'Found umatching symbols for {s}, just ignored for now!')
        else:
...
def dbn2idx(dbn, length=None, padding_idx=-100, dtype=np.int32):
    """ convert a dot-bracket notation to a list of indices
        length: the length of the sequence, if not given, use the length of dbn
    """
    ...
    if length is None or length == len(idx):
        return idx

    if length > len(idx):
        idx = np.concatenate([idx, np.full(length - len(idx), padding_idx, dtype=dtype)], axis=0)
    else:
        idx = idx[:length]
...
def dbn2onehot(dbn, length=None, open_close=False, use_nn=0, dbn2vec_dict=dbn2onehot_dict):
    """ use_nn: include the neighboring residues to encode the current one 
        open_close: include the open/close indicator as a two-channel feature
    """
    dbn_len = len(dbn)
    if not length:
        length = dbn_len
    ...
    if not use_nn:
        dbn_emb = [dbn2vec_dict[dbn[i]] for i in range(min([length, dbn_len]))]
    else:
        use_nn = int(use_nn)
        dbn_capped = ('-' * use_nn) + dbn + ('-' * use_nn)
        nn_indices = list(range(-use_nn, use_nn + 1))

        dbn_emb = [np.concatenate([dbn2vec_dict[dbn_capped[i + _i]] for _i in nn_indices], axis=0)
            for i in range(use_nn, min([length, dbn_len]) + use_nn)]

    if open_close:
        dbn_capped = f'-{dbn}-'
        open_close = [[dbn_capped[i] in '({[<' and dbn_capped[i-1] not in '({[<',
                       dbn_capped[i] in ')}]>' and dbn_capped[i+1] not in ')}]>'] 
                       for i in range(1, len(dbn_capped) - 1)]
        _ = [_l.append(not any(_l)) for _l in open_close]
        
        dbn_emb = [np.concatenate([dbn_emb[i], 
                                   np.array(open_close[i], dtype=dbn_emb[0].dtype)], axis=0)
                                   for i in range(len(dbn_emb))]

    if length > len(dbn_emb):
        dbn_emb.extend(np.zeros((length - len(dbn_emb), dbn_emb[0].shape[0]), dtype=dbn_emb[0].dtype))

    dbn_emb = np.stack(dbn_emb, axis=0)
    return dbn_emb
    

def dbn2topology(dbn, length=None, onehot=False, padding_idx=-100, dtype=np.int32):
    """ only determine three states: stem openning, stem closing, and neither
    """

    dbn_capped = f'-{dbn}-'

    topo_emb = [ 0 if dbn_capped[i] in '({[<' and dbn_capped[i-1] not in '({[<' else
                 1 if dbn_capped[i] in ')}]>' and dbn_capped[i+1] not in ')}]>' else
                 2 for i in range(1, len(dbn_capped) - 1) ]

    topo_emb = np.array(topo_emb, dtype=dtype)

    if onehot:
        identity_mat = np.eye(3, dtype=dtype)
        topo_emb = identity_mat[topo_emb]

    if length is None or length == len(topo_emb):
        return topo_emb
    
    elif length > len(topo_emb):
        if onehot:
            topo_emb = np.concatenate([topo_emb, np.zeros((length - len(topo_emb), 3), dtype=dtype)], axis=0)
        else:
            topo_emb = np.concatenate([topo_emb, np.full(length - len(topo_emb), padding_idx, dtype=dtype)], axis=0)
    else:
        topo_emb = topo_emb[:length]
...
def ct2dbn(ct, l=None):
    """ ct should be nx2 numpy array 
        Warning: only (.) are used, not the extended notations!
    """
    if l is None: l = ct.max()
...
def ct2ctmat(ct, l=None, dtype=np.int32):
    ...
    # check if ct is a numpy array
    if not isinstance(ct, np.ndarray):
        logger.error(f'ct is not a numpy array: {ct}')
        return None
...
def remove_noncanonical_in_ct(ct, seq, canonicals=['AU', 'CG', 'GU']):
    """ canonical base pairs must be sorted, i.e., AU not UA """
    if len(ct) == 0 or len(seq) == 0:
        logger.warning(f'Empty ct:{ct} and/or seq:{seq}!')
        return ct
    ...
    bptypes = np.char.add(seq_ct[:,0], seq_ct[:,1])
    idx2keep = np.concatenate([np.where(bptypes == _bp)[0] for _bp in canonicals])
    return ct[idx2keep]


def remove_pseudoknot_by_nkn(dbn, nkn, return_nkn=False):
    """ ssn notation appears not to contain information about pseudoknots """
    # find K in nkn notation
    nkn = np.fromiter(nkn, (np.compat.unicode, 1))
    iknots = np.where(nkn == 'K')[0]

    if len(iknots):
        dbn = np.fromiter(dbn, (np.compat.unicode, 1))
        dbn[iknots] = '.'
        dbn = ''.join(dbn.tolist()) # dbn.tostring().decode('utf-8') # np.char.join(dbn)
        if return_nkn:
            nkn[iknots] = 'N'
            nkn = ''.join(nkn.tolist()) # nkn.tostring() # np.char.join(nkn)
...
def count_ct_bptypes(ct, seq, return_tuple=True):
    """ Count the types of paired bases in ct and seq
        pairing residues are sorted, e.g., UA and AU are assigned to AU
    """
...
def parse_res2res_mat2d_lines(dist_info, dtype=np.float32, return_list=False):
    """ 
    This is the self-defined res2res 2D matrix tablular data. It should work for
    inter-residue distance, dihedral angle
    current columns are:
        resID | nts+resnum... 
    """

    try:
        if isinstance(dist_info, str) or isinstance(dist_info, Path):
            fname = str(dist_info)
            dist_lines = get_file_lines(fname, keep_empty=False, comment='#', keep_comment=False)
        else:
            fname = ''
            dist_lines = dist_info
        df = pd.read_csv(StringIO('\n'.join(dist_lines)), sep=r'\s+', skiprows=None,
                header='infer', engine='c')
        seq_dict = dict(fname=fname)
        seq_dict['resnames'] = df['resID'].str[0].sum()
        seq_dict['resnums'] = df['resID'].str[1:].astype(int).to_list()
        seq_dict['mat2d'] = df.iloc[:, 1:].to_numpy(dtype=dtype)
                
    except:
        logger.error(f'Error in parsing file: {fname}!!!')
        return None

    if return_list:
        return seq_dict['fname'], seq_dict['resnames'], seq_dict['resnums'], seq_dict['mat2d']
    else:
        return seq_dict


def mat2quant(mat_data, start=2, end=34, width=1, underflow=True, overflow=True, 
        onehot=False, keepdim=False, dtype=np.int64):
    """ mat_data must be np.ndarray """
    ...
    mat_idx = ((mat_data - start) / width).round().astype(dtype)
    onehot_dim = int((end - start) / width) + 1
    ...
    if overflow:
        mat_idx[mat_idx > onehot_dim] = onehot_dim
        onehot_dim += 1
    
    if onehot:
        mat_onehot = np.zeros(mat_idx.shape + (onehot_dim,), dtype=dtype)
        mat_onehot[(*np.indices(mat_idx.shape, sparse=True), mat_idx)] = 1

        return mat_onehot
    else:
        if keepdim:
            return np.expand_dims(mat_idx, -1)
        else:
            return mat_idx
...
class SeqStruct2D():
    ...
    def parse_files(self, fnames, fmt=None, istart=None,
        ...
        if fmt in ['fa', 'fasta', 'seq', 'seqres', 'refseq']:
            ...
            parse_func = functools.partial(parse_st_lines, fmt=fmt, seq=True,
                    dbn=True, ssn=True, nkn=True)
        elif fmt in ['sto', 'stk', 'stockholm']:
            parse_func = functools.partial(parse_stockholm_lines, fmt=fmt)
        elif fmt == 'ct':
        ...
        if num_files > 7:
            ...
            with mpi.Pool(processes=num_cpus) as mpool:
                ...
                parsed_seq_dicts = list(tqdm(mpool.imap(parse_func, fnames), total=len(fnames), desc='Parsing files'))
                if database is not None:
                    database = database.lower()
                    moltypes = list(tqdm(mpool.imap(functools.partial(parse_moltype, database=database), fnames),
                        total=len(fnames), desc='Parsing moltypes'))
        else:
        ...
        if fmt in ['seqres']:
            ...
            refseq_dicts = misc.mpi_map(parse_refseq_id,
                [_seq_dict['id'] for _seq_dict in parsed_seq_dicts])
            assert len(refseq_dicts) == len(parsed_seq_dicts), 'refseq and seq dicts must have the same length!'
            for i in range(len(parsed_seq_dicts)):
                parsed_seq_dicts[i].update(refseq_dicts[i])            
    ...
    def to_df(self, idx_start=1, seq=True, dbn=False, ssn=False, nkn=False, ct=False, tangle=False, has=False,
        ...
        dict2df = dict(
            idx = list(range(idx_start, num_seqs + idx_start)),
        ...
        if dbn:
            dict2df['dbn'] = self.dbn
        if ssn:
            dict2df['ssn'] = self.ssn
        if nkn:
            dict2df['nkn'] = self.nkn
        if ct:
            dict2df['ct'] = self.ct
